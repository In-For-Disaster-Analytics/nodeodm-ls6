        ...
        if not io.file_exists(alignment_file) or rerun:
            ...
            for attempt in range(1, max_attempts + 1):
                try:
                    ...
                    is_remote_io_error = getattr(e, 'errno', None) == 121 or 'Remote I/O error' in str(e)
                    if not is_remote_io_error or attempt >= max_attempts:
                        raise

                    sleep_seconds = min(60, 5 * (2 ** (attempt - 1)))
                    log.ODM_WARNING(
                        "OpenSfM submodel alignment failed with transient remote I/O error "
                        "(attempt %s/%s): %s. Retrying in %ss..." %
                        (attempt, max_attempts, str(e), sleep_seconds)
                    )
