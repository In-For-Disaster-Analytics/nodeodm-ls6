...
class Task:
    ...
    def execute_remote_task(self, done, seed_files = [], seed_touch_files = [], outputs = [], name_override = None):
        ...
        log.ODM_INFO("LRE DEBUG: patched remote.py active for %s; ODM_REMOTE_USE_IMPORT_PATH=%s ODM_IMPORT_PATH_BASE=%s NODEODM_IMPORT_PATH_ROOTS=%s _tapisJobWorkingDir=%s project_path=%s" %
                     (self,
                      os.environ.get("ODM_REMOTE_USE_IMPORT_PATH", ""),
                      os.environ.get("ODM_IMPORT_PATH_BASE", ""),
                      os.environ.get("NODEODM_IMPORT_PATH_ROOTS", ""),
                      os.environ.get("_tapisJobWorkingDir", ""),
                      self.project_path))
        ...
        if use_import_path:
            log.ODM_INFO("LRE DEBUG: import_path mode enabled for %s; node=%s node_type=%s has_create_task_from_path=%s" %
                         (self, getattr(self.node, "url", self.node), type(self.node).__name__, hasattr(self.node, "create_task_from_path")))
            rel = None
            try:
                ...
                rel = None
            log.ODM_INFO("LRE DEBUG: import_path rel/base for %s: rel=%s base=%s exists_project=%s" %
                         (self, rel, import_path_base, os.path.exists(self.project_path)))
            ...
            if candidate_base and rel:
                try:
                    ...
                    candidate_path = None
            log.ODM_INFO("LRE DEBUG: import_path candidate for %s: candidate_base=%s candidate_path=%s exists=%s" %
                         (self, candidate_base, candidate_path, os.path.exists(candidate_path) if candidate_path else False))
            ...
            log.ODM_INFO("LRE DEBUG: import_path target before flatten for %s: %s exists=%s" %
                         (self, target_import_path, os.path.exists(target_import_path)))
            import_path_override = flatten_import_path(
                target_import_path,
                extra_seed_files=seed_files,
                extra_touch_files=seed_touch_files,
            )
            log.ODM_INFO("LRE DEBUG: import_path target after flatten for %s: %s exists=%s" %
                         (self, import_path_override, os.path.exists(import_path_override)))
        ...
        if task is None:
            if use_import_path:
                log.ODM_WARNING("LRE DEBUG: import_path mode was enabled for %s but task is None before seed.zip fallback; this should not happen" % self)
            else:
                log.ODM_INFO("LRE DEBUG: import_path mode disabled for %s; using seed.zip upload path" % self)
            seed_attempt = 0
            max_seed_retries = 1
